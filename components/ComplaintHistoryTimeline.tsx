...
import { DocumentTextIcon, CogIcon, SparklesIcon, CheckCircleIcon, UserIcon } from '../constants';

interface ComplaintHistoryTimelineProps {
  history: Complaint['history'];
}

const getStatusInfo = (status: ComplaintStatus, actor: 'Citizen' | 'Admin' | 'System') => {
    if (actor === 'Citizen') {
        return { Icon: UserIcon, color: 'bg-indigo-500' };
    }
    switch (status) {
        ...
        case ComplaintStatus.REOPENED: return { Icon: UserIcon, color: 'bg-indigo-500' };
        ...
        <div className="mt-4">
            <ol className="relative border-l border-neutral-gray ml-3">
                {[...history].reverse().map((event, index) => {
                    const { Icon, color } = getStatusInfo(event.status, event.actor);
                    return (
                        <li key={index} className="mb-6 ml-6">
                            ...
                            <div className="ml-2">
                                <h3 className="font-semibold text-neutral-dark-gray text-sm flex items-center gap-2">
                                   {event.actor}'s Update <span className="font-normal text-gray-500 text-xs">({event.status})</span>
                                </h3>
